...
Controller::Controller(IPort& p_displayPort, IPort& p_foodPort, IPort& p_scorePort, std::string const& p_config)
    : m_displayPort(p_displayPort),
      ...
      m_paused(false),
      food{p_displayPort, p_foodPort, this}  
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        ...
        food.m_foodPosition = std::make_pair(foodX, foodY);
...
bool ControllMap::isPositionOutsideMap(int x, int y) const
{
    return x < 0 or y < 0 or x >= m_mapDimension.first or y >= m_mapDimension.second;
}

void ControllFood::sendPlaceNewFood(int x, int y)
{
...
void ControllFood::sendClearOldFood()
{
...
{
    if (std::make_pair(newHead.x, newHead.y) == food.m_foodPosition) {
...
void ControllFood::updateFoodPosition(int x, int y, std::function<void()> clearPolicy)
{
    if (control->isSegmentAtPosition(x, y) || control->map.isPositionOutsideMap(x,y)) {
        m_foodPort.send(std::make_unique<EventT<FoodReq>>());
        return;
    }
...
void Controller::handleFoodInd(std::unique_ptr<Event> e)
{
    ...
    food.updateFoodPosition(receivedFood.x, receivedFood.y, std::bind(&food.sendClearOldFood, this));
}
...
void Controller::handleFoodResp(std::unique_ptr<Event> e)
{
    ...
    food.updateFoodPosition(requestedFood.x, requestedFood.y, []{});
}
